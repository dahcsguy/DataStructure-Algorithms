...
public class Formula {
	...
	private int tempform [][]; //copy of the original form
    ...
    */
	...
	// TODO
		...
		tempform = new int [nclauses][MAXCLAUSELEN];//copy of the original form
		...
		for(int i = 0 ; i< form.length;i++){
			ll.add(i); //add index to clauseStack
		}
    ...
    */
    boolean hasEmptyClause () {
		// TODO
		int i = 0;
		while(i<form.length){//iterate from 0 to n-1 of form
			if(isEmptyClause(i)) //if there's an empty clause at i
				return true;
			i++; //increment
		}
		return false;
    ...
    */
	...
	// TODO
		...
		return true; //all of elements in row c are 0
    }
    /**
    ...
    */
	...
	// TODO
		for(int i = 0; i<tempform[clz].length;i++){//iterate through row clz
			if(tempform[clz][i] ==(var)){ //if there is var in the formula
				return true;
    ...
    */
	...
	// TODO
		...
		if(val){
			...
			tempVar = var; //its not false so var
		}
		else{
			vars[var] = -1; //vars[var] is set to false
			tempVar = -var; //its false so -var
		}
			
		
		int opposite = -tempVar;//variable to mark the opposite of var
		
		
		LinkedList<Integer> theList = clauseStack.peek();
		...
		while(iter.hasNext()){ //iterate through the ll at the top of stack
			...
			if(!inClause(rowNum,tempVar)){//tempvar is not in form[rowNum] so we have to add it to the ll
				newList.add(rowNum); //add to ll
				if(inClause(rowNum,opposite)){ //opposite variable is in form[rowNum]
    ...
    */
	...
	// TODO
		...
		LinkedList<Integer> thePop = clauseStack.pop();
		int opposite = -var;
		...
		while(iter.hasNext()){ //iterate through the linkedlist at the top of the stack
			...
			if(inClause(rowNum,opposite)||inClause(rowNum,var)){//if the opposite or the actual variable is in the clause
				for(int i = 0; i< tempform[rowNum].length;i++){ //iterate through the array 
					if(tempform[rowNum][i]==opposite||tempform[rowNum][i]==var){ //find the instances at which they match
						form[rowNum][i] = tempform[rowNum][i]; //backtrack
					}
